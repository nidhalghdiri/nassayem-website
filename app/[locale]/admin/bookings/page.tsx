...
export default async function AdminBookingsPage({
  ...
  // Build filter from URL param
  const activeFilter = rawStatus?.toUpperCase() ?? "ALL";
  const whereClause =
    activeFilter !== "ALL"
      ? { status: activeFilter as BookingStatus }
      : {};
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
        {filterTabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.key}
              href={`/${locale}/admin/bookings${tab.key === "all" ? "" : `?status=${tab.key}`}`}
          ...
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
            ...
            <p className="text-gray-500 text-sm max-w-xs">
              {activeFilter !== "ALL"
                ...
                : isEn
                  ? "When guests make a reservation, it will appear here."
                  : "عندما يقوم الضيوف بحجز، ستظهر هنا."}
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-start border-collapse">
                <thead>
                  <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100">
                    <th className="px-6 py-4 font-semibold text-start">{isEn ? "Guest" : "الضيف"}</th>
                    <th className="px-6 py-4 font-semibold text-start">{isEn ? "Property" : "العقار"}</th>
                    <th className="px-6 py-4 font-semibold text-start">{isEn ? "Dates" : "التواريخ"}</th>
                    <th className="px-6 py-4 font-semibold text-start">{isEn ? "Amount" : "المبلغ"}</th>
                    <th className="px-6 py-4 font-semibold text-start">{isEn ? "Status" : "الحالة"}</th>
                    <th className="px-6 py-4 font-semibold text-end">{isEn ? "Details" : "التفاصيل"}</th>
                  </tr>
                </thead>
                  ...
                  {bookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <span className="text-xs text-gray-400 ml-1">{isEn ? "OMR" : "ر.ع"}</span>
                      </td>
                      <td className="px-6 py-4">
