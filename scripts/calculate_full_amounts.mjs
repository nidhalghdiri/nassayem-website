...
async function main() {
  ...
  console.log("--------------------------------------------------------------------------------");
  console.log(`💰 TOTAL ADVANCE DEPOSIT PAID ONLINE:             ${totalAdvancePaid.toFixed(3)} OMR`);
  ...
  const exportRows = detailedResults.map((r, i) => [
    i + 1,
    `"${r.ref}"`,
  ...
  console.log("Saved results CSV to: scripts/chatbot_95_reservations_full_value.csv\n");
