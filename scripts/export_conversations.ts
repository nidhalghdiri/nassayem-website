...
async function exportConversations() {
  console.log("Connecting to database to export chatbot conversations...");
  ...
  const allRefs = Array.from(new Set(refByConv.values())).filter(Boolean);
  ...
  const paymentByRef = new Map<string, { status: string; amount?: number; paidAt?: Date | null }>();
  ...
  for (const c of conversations) {
    ...
    if (c.status in statusStats) {
      statusStats[c.status as keyof typeof statusStats]++;
    }
  ...
  const outputPath = path.join(process.cwd(), "nassayem_chatbot_conversations_export.md");
  fs.writeFileSync(outputPath, out.join("\n"), "utf-8");
  const fallbackPath = path.join(process.cwd(), "nassayem-chatbot-conversations.md");
  fs.writeFileSync(fallbackPath, out.join("\n"), "utf-8");
  console.log(`Successfully exported ${conversations.length} conversations to ${outputPath} and ${fallbackPath} (${(fs.statSync(outputPath).size / 1024 / 1024).toFixed(2)} MB)`);
}
